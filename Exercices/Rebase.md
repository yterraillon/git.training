...
Atteindre le master, et de l? créer et atteindre une branche "cible-rebase".
...
On va ? présent rebaser la branche 'cible-rebase' sur la branche 'nouvel-historique'
...
Observer ? présent le statut dans lequel on se trouve : il y est possible de faire un pull depuis l'origin. Si on pushe, le push va être rejeté.
...
La différence sera dans les commandes ? jouer après résolution :

`git rebase --continue` pour poursuivre le rebase
`git rebase --abort` pour revenir ? l'état d'avant rebase
...
Git propose d'abord de sélectionner les commits qui vont être retenus (via le m?me éditeur qu'on a déj? rencontré.)
...
Aller au début de sa ligne avec le curseur, appuyer sur la touche S une fois, puis suppr jusqu'? avoir supprimer le mot 'push'. Taper 'squash' ? la place,
